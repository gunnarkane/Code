...
class letterBag:
    '''
    This is a list of available letters for players to pick up
    from when they do not have 7 of there own.
    '''
    
    def __init__(self):
        '''
        Creates a List then adds all 98 letters to it
        '''
        self.letterSet = []
    ...
    def createLetterSet(self):
        '''
        Creates a fresh list with a full set of letters for Scrabble. The only letters
        not in the list are the blank letters. Those are not in this game.
    ...
    def lettersLeft(self):
        '''
        Returns the length of the letter set
        '''
        return len(self.letterSet)     
    
    def getLetters(self, number):
        '''
        Gets new letters to give to players who don't have 7. 
        @param: number is the number of letters the player needs
        '''
        lettersGiven = []
